<cfmodule template="#application.appPath#/header.cfm" title='Consultant Gallery'>
...
<cftry>
	...
	<cfelseif action EQ "vote">
		...
		<cfif getVotes.recordCount GTE 1>
			<cfthrow message="Error" detail="You have already voted for a consultant image.">
		</cfif>
...
<cfif hasMasks('consultant') AND getContest.open_vote>
	...
	<cfoutput>
		<p class="ok">You have #remVotes# consultant entry vote<cfif remVotes NEQ 1>s</cfif> remaining.
			<cfif remVotes LT 1>
				[<a href="#application.appPath#/tools/photoshop-contest/consultant-gallery.cfm?action=resetVotes"
					onClick="return(confirm('Are you sure you wish to reset your consultant entry vote?'))">Reset Votes</a>]		
			</cfif>
		</p>
	</cfoutput>
...
<cfif userView>
	<h2>My Entries</h2>
<cfelse>
	<h2>Consultant Entry Gallery</h2>
</cfif>
...
<cfmodule template="#application.appPath#/tools/photoshop-contest/mod-gallery.cfm" 
		  ...
		  gallery_url="consultant-gallery.cfm">
